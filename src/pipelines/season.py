...
class SeasonPipeline(AnalysisPipeline):
    ...
    def run(self) -> bool:
        ...
        # --- Phase 3: Analysis (Per Club Reports) ---
        if not self.args.skip_club_reports:
            self.log("Phase 2: Generating Per-Club Reports...")
            if not self._generate_club_reports(cleaned_df, league):
                self.log("Club reporting encountered errors (see logs).", logging.WARNING)
        else:
             self.log("Skipping Per-Club Reports.")

        # --- Phase 4: League Wide Report ---
        self.log(f"Phase 3: Generating League-Wide {timeframe.title()} Report...")
        if not self._generate_league_report(cleaned_df, league, timeframe):
            return False
            
        self.log("Season Pipeline Complete!")
        return True

    def _generate_club_reports(self, df: pd.DataFrame, league: str) -> bool:
        ...
        try:
            ...
            for club in unique_clubs:
                try:
                    ...
                    save_document(doc, str(reports_dir), f"{club}_report.docx")
                    count += 1
    ...
    def _generate_league_report(self, df: pd.DataFrame, league: str, timeframe: str) -> bool:
