...
The pipeline follows a **4-phase orchestrated architecture** coordinated by `src/pipeline/orchestrator.py`:
...
- **`src/pipeline/`**: `PipelineExecutor` class orchestrates all phases with logging
...
4. Call in `src/pipeline/orchestrator.py` Phase 4
...
- Error handling logs via `logging` module (see `orchestrator.py` for patterns)
